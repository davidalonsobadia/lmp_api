...
-----------------------------------------------
	...
	-- Service Providers
INSERT INTO service_provider(name, description, logo)
	VALUES('PIDaaS', 'PIDaaS Service provider', 'no-logo');
	
INSERT INTO service_provider(name, description, logo)
	VALUES('TIC-Salut', 'TIC-SALUT description', 'ticsalut_logo1.gif');

INSERT INTO service_provider(name, description, logo)
	VALUES('CSI Piemonte', 'CSI Description', 'csi_piemonte_logo1.gif');

INSERT INTO service_provider(name, description, logo)
	VALUES('Ebros', 'This is going to be a long description for Ebros to see how it mathces on the screen.', 'ebros_logo1.gif');

	-- SANs
INSERT INTO SAN(name, enabled)
	VALUES ('PIDaaS User', true);
INSERT INTO SAN(name, enabled)
	VALUES ('SAN TIC-Salut', true);
INSERT INTO SAN(name, enabled)
	VALUES ('SAN TICSalut 2', true);
INSERT INTO SAN(name, enabled)
	VALUES ('SAN CSI', true);
INSERT INTO SAN(name, enabled)
	VALUES ('SAN CSI 2', true);
INSERT INTO SAN(name, enabled)
	VALUES ('SAN Ebros 1', true);
INSERT INTO SAN(name, enabled)
	VALUES ('SAN Ebros 2', true);
	
	-- service_provider_has_Sans
INSERT INTO service_provider_has_Sans(service_provider_id, San_id)
	SELECT sp.id, s.id
	FROM service_provider sp
	JOIN SAN s
		ON s.name LIKE 'PIDaaS User'
	WHERE sp.name LIKE 'PIDaaS';
INSERT INTO service_provider_has_Sans(service_provider_id, San_id)
	SELECT sp.id, s.id
	FROM service_provider sp
	JOIN SAN s
		ON s.name LIKE 'SAN TIC-Salut'
	WHERE sp.name LIKE 'TIC-Salut';
INSERT INTO service_provider_has_Sans(service_provider_id, San_id)
	SELECT sp.id, s.id
	FROM service_provider sp
	JOIN SAN s
		ON s.name LIKE 'SAN TIC-Salut 2'
	WHERE sp.name LIKE 'TIC-Salut';
INSERT INTO service_provider_has_Sans(service_provider_id, San_id)
	SELECT sp.id, s.id
	FROM service_provider sp
	JOIN SAN s
		ON s.name LIKE 'SAN CSI'
	WHERE sp.name LIKE 'CSI Piemonte';
INSERT INTO service_provider_has_Sans(service_provider_id, San_id)
	SELECT sp.id, s.id
	FROM service_provider sp
	JOIN SAN s
		ON s.name LIKE 'SAN CSI 2'
	WHERE sp.name LIKE 'CSI Piemonte';
INSERT INTO service_provider_has_Sans(service_provider_id, San_id)
	SELECT sp.id, s.id
	FROM service_provider sp
	JOIN SAN s
		ON s.name LIKE 'SAN Ebros 1'
	WHERE sp.name LIKE 'Ebros';
INSERT INTO service_provider_has_Sans(service_provider_id, San_id)
	SELECT sp.id, s.id
	FROM service_provider sp
	JOIN SAN s
		ON s.name LIKE 'SAN Ebros 2'
	WHERE sp.name LIKE 'Ebros';
	
	-- person_has_service_providers
INSERT INTO person_has_service_providers(person_id, service_provider_id)
	SELECT p.id, sp.id
	FROM service_provider sp
	JOIN person p
		ON p.email LIKE @user1Mail
	WHERE sp.name LIKE 'PIDaaS';
INSERT INTO person_has_service_providers(person_id, service_provider_id)
	SELECT p.id, sp.id
	FROM service_provider sp
